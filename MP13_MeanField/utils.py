...
visualize = True
perform_computation = (os.environ.get('NBGRADER_VALIDATING', '0') != '1') and ('/submitted/' not in os.path.abspath(__file__)) and ('/autograded/' not in os.path.abspath(__file__))
...
def show_test_cases(test_func, task_id=0):
    ...
    for i in range(ref_images.shape[0]):
        ...
        equality_array = np.isclose(test_image, ref_image, rtol=1e-05, atol=1e-08, equal_nan=True)
        if np.all(equality_array):
            if visualize_:
            ...
            ineq_idxs = np.array(np.where(np.logical_not(equality_array)))[:,0].tolist()
