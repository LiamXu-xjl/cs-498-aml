...
visualize = True
perform_computation = (os.environ.get('NBGRADER_VALIDATING', '0') != '1') and ('/submitted/' not in os.path.abspath(__file__))
...
def arg2str(args, kwargs, adv_user_msg=False):
    ...
    msg += f'Each argument will be printed on a seperate line: \n\n\n'
    for arg_ in args:
        msg += f'{arg_},\n'
    ...
    if adv_user_msg:
        msg += '\n\n---------\n'
        msg += 'For advanced users (optional):\n  In case you would rather not copy the arguments from above or the printing percision was causing distortions, the test results dictionary, which was returned, contains the following material :\n\n'
...
def test_case_checker(stu_func, task_id=0):
    out_dict = {}
    if not os.path.exists(f'test_db/task_{task_id}.npz'):
    ...
    for (test_args, test_kwargs, correct_sol) in test_case_loader(f'test_db/task_{task_id}.npz'):
        try:
...
def show_test_cases(test_func, task_id=0):
    ...
    file_path = f'test_db/task_{task_id}.npz'
    ...
    for i in range(ref_images.shape[0]):
        ...
        if visualize_:
            ax = axes[0,0]
            ax.pcolormesh(orig_image[::-1,:], edgecolors='k', linewidth=0.01, cmap='Greys')
            ax.set_aspect('equal')
            ax.set_title('Raw Image')

            ax = axes[0,1]
            ax.pcolormesh(ref_image[::-1,:], edgecolors='k', linewidth=0.01, cmap='Greys')
            ax.set_aspect('equal')
            ax.set_title('Reference Solution Image')

            ax = axes[0,2]
            ax.pcolormesh(test_image[::-1,:], edgecolors='k', linewidth=0.01, cmap='Greys')
            ax.set_aspect('equal')
